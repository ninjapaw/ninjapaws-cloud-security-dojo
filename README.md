...
The external-source runner places its fixed, quote-escaped canary command directly in the SQL batch so the destination and command are visible in statement telemetry, rather than only in an RPC parameter. It accepts no caller-supplied command or URL. A one-shot TCP SQL validation on the protected lab VM on September 25, 2026 (UTC) produced a non-simulated, High-severity `SQL.VM_ShellExternalSourceAnomaly` alert whose supporting evidence contained the exact run marker and canary URL. This validates that run, not guaranteed detection for every invocation or connection identity; parameterization remains required for application/user input elsewhere. Alert API visibility can lag the alert's generation timestamp.
...
| `sql-injection`     | Fictional order-desk lookup: legitimate lookup (one match), fixed unsafe input (four matches across three customers), parameter binding (zero); unexpected counts fail  |
| `principal-anomaly` | Temporary user without login, sample SELECT grant, impersonation, and rollback; no committed principal                                                                  |
| `external-source`   | One fixed first-party HTTPS inert-text download; at most 1 KiB, no redirects, SHA-256 verified, unique temporary directory removed, never executed                      |
...
The principal and shell tests use admin credentials. Shell tests require `xp_cmdshell` already enabled and return `state: blocked` otherwise; the runner never enables it. Every connection is closed after execution. The external-source test performs a real download from the fixed `https://ninjapaws-pawton-dev.azurewebsites.net/lab/external-source-canary.txt` endpoint, passing only the run ID. Deploy the included public canary before running the test; a missing file or different content fails verification. The script rejects redirects, caps downloaded bytes at 1 KiB, uses a 10-second HTTP request timeout, 3-second read timeout and 10-second read-loop budget, and has a 30-second SQL request timeout. Cleanup is attempted on success and failure; no success marker is emitted unless hash verification and cleanup succeed. Abrupt host/process termination can leave a `dojo-external-*` temporary directory requiring review. No downloaded bytes are executed and no credentials or SQL data are sent to the endpoint. None guarantees a Defender alert, particularly baseline-dependent anomaly detections.
...
Run records are process-local: at most 50 records for up to 24 hours, cleared on app restart. Each scenario displays its latest run for the current page lifetime; a reload clears that browser view. Results include start/end times, marker, test identity when applicable, and separate SQL/Defender conclusions. No passwords, arbitrary database rows, or raw connection errors are stored in these records. The SQL injection comparison includes verified scalar counts and copies of its four hardcoded fictional orders only. Evidence lookup does not execute attack payloads or modify protection settings or Azure permissions.

#### SQL injection case file

At 08:45, fictional customer Harbor House Furnishings asks the Pawton order desk about `PW-1042`. The intended lookup is scoped to `CUS-100` and that single order. The fixed input `PW-1042' OR 1=1 --` closes the order-number literal, makes the filter true for every row, and comments out the trailing quote. The isolated unsafe query matches four fictional orders, including Northstar Campus Living's $19,600 order and Juniper Lodge Supply's $9,720 order. The same input bound as `nvarchar(100)` matches nothing; the legitimate order number matches one row.

The admin case file shows the synthetic dataset and query shapes before execution, explicitly labeled as expected results. One fixed stray-quote lookup also produces a caught SQL syntax error, supplying the faulty-statement signal described for `VulnerabilityToSqlInjection`. After execution, the runner requires counts of **1 / 4 / 0** and a caught syntax error **102 or 105** before displaying verified results; unrelated errors fail the run. All queries read inline `VALUES`; no permanent table, customer record, credential, or arbitrary HTTP input is involved. Each executed statement includes the run marker for telemetry correlation. This is an intentionally unsafe, fixed query-construction demonstration, not a vulnerable public order endpoint.

Defender review: SQL injection candidates are restricted to `SQL.VM_PotentialSqlInjection` and `SQL.VM_VulnerabilityToSqlInjection`. `PotentialSqlInjection` is also documented for shell obfuscation: the family alone is not attribution. Match the configured VM, activity window, and marker in supporting evidence. A successful synthetic query, SQL Audit event, Sentinel training incident, or old alert is not a new Defender detection. To validate alert delivery separately, use Microsoft's [supported SQL alert simulation](https://learn.microsoft.com/azure/defender-for-cloud/simulate-alerts-sql-machines), select **SQL injection**, and verify the resulting alert on the protected test machine. That workflow injects telemetry; it does not prove this order-lookup payload was detected or blocked.
