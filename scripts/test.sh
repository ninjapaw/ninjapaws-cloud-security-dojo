...
file_contains "$REPO_ROOT/config/deploy.config.json" '"AgentlessVmScanning": "false"'
file_contains "$REPO_ROOT/config/deploy.config.json" '"ApiPosture": "false"'
...
file_contains "$REPO_ROOT/.github/workflows/deploy.yml" "DEFENDER_CSPM_VM_SCANNING: \${{ vars.DEFENDER_CSPM_VM_SCANNING || 'false' }}"
file_contains "$REPO_ROOT/.github/workflows/deploy.yml" "DEFENDER_CSPM_API_POSTURE: \${{ vars.DEFENDER_CSPM_API_POSTURE || 'false' }}"
file_contains "$REPO_ROOT/.github/workflows/deploy.yml" "DEFENDER_CONTAINERS_VM_SCANNING: \${{ vars.DEFENDER_CONTAINERS_VM_SCANNING || 'false' }}"
...
# prompt_region() moved to lib/common.sh so deploy.sh and setup-azure-github-oidc.sh share one copy.
file_contains "$REPO_ROOT/scripts/lib/common.sh" 'Select a region by number or name'
...
for variable_name in BASE_OS_IMAGE BASE_OS_VERSION NGINX_VERSION NODE_MAJOR_VERSION VULNERABILITY_STATUS PORT NPM_REGISTRY_URL NPM_USE_MIRROR NPM_NETWORK_MODE DEFENDER_ENABLED DEFENDER_SCAN_ENABLED DEFENDER_MANAGE_PLANS DEFENDER_TARGET_CVE DEFENDER_APPSERVICES_TIER DEFENDER_CONTAINERS_TIER DEFENDER_CSPM_TIER DEFENDER_ARM_TIER DEFENDER_MANAGE_EXTENSIONS DEFENDER_CSPM_SERVERLESS_PROTECTION DEFENDER_CSPM_SERVERLESS_CONTAINERS DEFENDER_CSPM_REGISTRY_ASSESSMENT DEFENDER_CONTAINERS_REGISTRY_ASSESSMENT DEFENDER_DEVOPS_CONNECTOR_ENABLED ADVANCED_SECURITY_EXPECTED; do
